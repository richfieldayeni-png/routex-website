...
const categories = [
  { title: 'Getting Started', icon: '🚀', faqs: [
    ...
    { q: 'Is RouteX available to individuals or only companies?', a: 'RouteX primarily serves corporate passengers. Individual commuters can also sign up independently. Contact hello@routexgo.co to find out if your route is available.' },
    { q: 'Which areas in Abuja does RouteX serve?', a: 'We currently operate routes across Wuse Zone 2, Maitama, Garki, the Central Business District, and Kubwa corridors. Check the app for the latest available routes.' },
  ...
  { title: 'Booking a Ride', icon: '🎫', faqs: [
  ...
  { title: 'Trips & Tracking', icon: '📍', faqs: [
  ...
  { title: 'Cancellations & Refunds', icon: '↩️', faqs: [
  ...
  { title: 'Account & Profile', icon: '👤', faqs: [
    ...
    { q: 'How do I delete my account?', a: 'Email support@routexgo.co from your registered email address to request account deletion. Any pending refunds must be resolved first.' },
  ...
  { title: 'Corporate Accounts', icon: '🏢', faqs: [
    { q: 'How does my company sign up?', a: 'Visit routexgo.co, scroll to For Business, and submit an enquiry. Our team will reach out within 24 hours to discuss requirements, routes, and pricing.' },
    ...
    { q: 'Can we request a custom route?', a: 'Yes. Contact hello@routexgo.co with your office address and the areas where most employees live. We will assess feasibility and get back to you.' },
  ]},
  { title: 'Safety & Conduct', icon: '🛡️', faqs: [
  ...
  return (
    <>
      ...
      <div style={{ background: 'var(--green)', padding: '64px 5% 56px', textAlign: 'center' }}>
        <h1 style={{ fontFamily: 'var(--display)', fontSize: '40px', fontWeight: 800, color: 'white', letterSpacing: '-1px', marginBottom: '12px' }}>Frequently Asked Questions</h1>
        <p style={{ fontSize: '16px', color: 'rgba(255,255,255,0.75)', maxWidth: '560px', margin: '0 auto 28px' }}>
          ...
          <a href="/support" style={{ color: 'rgba(255,255,255,0.9)', textDecoration: 'underline' }}>Contact our support team</a>.
        </p>
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
          {categories.map((cat) => (
            <a key={cat.title} href={`#${cat.title.toLowerCase().replace(/\s+/g, '-').replace(/[&]/g, 'and')}`}
              style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '6px 14px', borderRadius: '20px', fontSize: '13px', fontWeight: 500 }}>
              {cat.icon} {cat.title}
            </a>
          ))}
        </div>
      ...
      <div style={{ maxWidth: '860px', margin: '0 auto', padding: '64px 24px 96px' }}>
        {categories.map((cat) => (
          <div key={cat.title} id={cat.title.toLowerCase().replace(/\s+/g, '-').replace(/[&]/g, 'and')} style={{ marginBottom: '56px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px', paddingBottom: '16px', borderBottom: '2px solid var(--green-lt)' }}>
              <span style={{ fontSize: '24px' }}>{cat.icon}</span>
              <h2 style={{ fontFamily: 'var(--display)', fontSize: '22px', fontWeight: 700, color: 'var(--black)', letterSpacing: '-0.5px' }}>{cat.title}</h2>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {cat.faqs.map((faq, i) => (
                <details key={i} style={{ background: 'var(--gray5)', borderRadius: '10px', border: '1px solid #E8EEF2', overflow: 'hidden' }}>
                  <summary style={{ padding: '18px 20px', fontSize: '15px', fontWeight: 600, color: 'var(--black)', cursor: 'pointer', listStyle: 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    {faq.q}
                    <span style={{ color: 'var(--green)', fontSize: '20px', flexShrink: 0, marginLeft: '12px' }}>+</span>
                  </summary>
                  <div style={{ padding: '0 20px 18px', fontSize: '14px', color: 'var(--gray2)', lineHeight: 1.7 }}>{faq.a}</div>
                </details>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: 'var(--green)', borderRadius: '16px', padding: '40px', textAlign: 'center' }}>
          <h3 style={{ fontFamily: 'var(--display)', fontSize: '24px', fontWeight: 800, color: 'white', letterSpacing: '-0.5px', marginBottom: '10px' }}>Still have questions?</h3>
          <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.75)', marginBottom: '24px' }}>Our support team is available Monday to Friday, 8am to 6pm WAT.</p>
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="/support" style={{ background: 'white', color: 'var(--green)', padding: '12px 24px', borderRadius: '8px', fontSize: '14px', fontWeight: 600 }}>Visit Support Centre</a>
            <a href="mailto:support@routexgo.co" style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '12px 24px', borderRadius: '8px', fontSize: '14px', fontWeight: 500 }}>Email Us</a>
          </div>
        </div>
      </div>
      <Footer />
